...
export async function useMarketsInfo(
  chainId: number,
  account: string,
  fetchBalance: boolean = false
): Promise<MarketsInfoResult> {
  // const { account } = useWallet();
  const { marketsData, marketsAddresses } = await useMarkets(chainId);
  const { tokensData, pricesUpdatedAt } = await useTokensData(
    chainId,
    account,
    fetchBalance
  );
  ...
  const { data } = await useMulticall(chainId, "useMarketsInfo", {
    key: [],
    request: () =>
    ...
    parseResponse: (res: any) => {
      return marketsAddresses!.reduce((acc: MarketsInfoData, marketAddress) => {
        ...
        acc[marketAddress] = {
          ...
          maxLongPoolAmountForDeposit: BigNumber.from(
            dataStoreValues.maxLongPoolAmountForDeposit.returnValues[0]
          ),
          ...
          totalBorrowingFees: BigNumber.from(
            poolValueInfoMax.totalBorrowingFees
          ),
          ...
          claimableFundingAmountShort:
            dataStoreValues.claimableFundingAmountShort
              ? BigNumber.from(
                  dataStoreValues.claimableFundingAmountShort?.returnValues[0]
                )
              : undefined,
