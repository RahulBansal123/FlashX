...
export const PRECISION = BigNumber.from(10).pow(30);
...
export const CHAIN_NAMES_MAP: { [key: number]: string } = {
  [BCS_MAINNET]: "BSC",
...
export const HIGH_EXECUTION_FEES_MAP: { [key: number]: number } = {
  [ARBITRUM]: 3, // 3 USD
  [AVALANCHE]: 3, // 3 USD
  [AVALANCHE_FUJI]: 3, // 3 USD
};

export const EXECUTION_FEE_MULTIPLIER_MAP: { [key: number]: number } = {
...
const constants: { [key: number]: any } = {
  [BCS_MAINNET]: {
...
// export const FALLBACK_PROVIDERS: { [key: number]: string[] } = {
